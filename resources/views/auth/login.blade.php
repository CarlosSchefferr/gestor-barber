<x-guest-layout>
    ...
    <div class="mb-8">
        <p class="text-xs font-semibold uppercase tracking-[0.22em] text-barber-500">Acesso ao painel</p>
        <h1 class="mt-3 text-3xl font-bold leading-tight text-zinc-900">Entrar na sua conta</h1>
        <p class="mt-2 text-sm text-zinc-600">Gerencie agenda, serviços e caixa em poucos cliques.</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="mt-5 flex items-center justify-between gap-3">
            <label for="remember_me" class="inline-flex cursor-pointer items-center gap-2 text-sm text-zinc-600">
                <input id="remember_me" type="checkbox" name="remember" class="rounded border-zinc-300 text-barber-500 focus:ring-barber-500/30">
                <span>Lembrar de mim</span>
            </label>

            @if (Route::has('password.request'))
                <a class="text-sm font-semibold text-barber-600 transition hover:text-barber-700" href="{{ route('password.request') }}">
                    Esqueceu a senha?
                </a>
            @endif
        </div>
